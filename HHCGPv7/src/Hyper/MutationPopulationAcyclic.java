 ...
 */
public class MutationPopulationAcyclic 
{
    ...
    private MutationPerformanceAcyclic MutationFitnessValue;

    public MutationPopulationAcyclic(int noMaxFitnessValues, String aFileName)
    {
        ...
        this.typeOfMutation = MutationPopulationAcyclic.LARGE_SCALE;
        this.bestFitnessValue =  Double.NEGATIVE_INFINITY;
        this.MutationFitnessValue = new MutationPerformanceAcyclic(noMaxFitnessValues);
    ...
    public void initAndEvalMutationOperators(OnlineAcyclicCoordinates CGPGraph, int generation) throws CloneNotSupportedException, IOException
    {
        ...
        for (short type = MutationPopulationAcyclic.SMALL_SCALE; type <= MutationPopulationAcyclic.RUIN_RECREATE; type++)
        {
    ...
    public MutationPerformanceAcyclic getMutationFitnessValues()
    {
        return this.MutationFitnessValue;
    }
    
    public void evaluateAMutation(OnlineAcyclicCoordinates aParent, OnlineAcyclicCoordinates anOffspring, int iteration, int GapSinceLastUpdate)
    {
    ...
    public void learningAMutation(OnlineAcyclicCoordinates aParent, OnlineAcyclicCoordinates anOffspring, int iteration, int GapSinceLastUpdate) throws CloneNotSupportedException, IOException 
    {
        short saveType; 
        int i;
           ...
           if (!this.MutationFitnessValue.hasATerriblePerformance())
           {
                if (GapSinceLastUpdate < (3 * this.MutationFitnessValue.getMutationFitnessValues().length))
                {
                    ...
                    while(i < 3) //3 & saveMutationFitness >=this.getVariationOperator().getAFitnessValue(0))
                    {
                        ...
                        {
                            this.mutateNewMutations((OnlineAcyclicCoordinates) aParent.clone());
                        }
    ...
    private void checkCurrentMutations(OnlineAcyclicCoordinates CGPGraph) throws CloneNotSupportedException, IOException
    {
        short saveType = this.typeOfMutation;
        short type = MutationPopulationAcyclic.SMALL_SCALE;
        boolean stop = false;
        while(!stop)
        {
            ...
            type++;
            stop = (type > MutationPopulationAcyclic.RUIN_RECREATE);
            if (!stop)
            {
    ...
    private void mutateNewMutations(OnlineAcyclicCoordinates CGPGraph) throws CloneNotSupportedException, IOException
    {
        short saveType = this.typeOfMutation;
        short type = MutationPopulationAcyclic.SMALL_SCALE;
        boolean stop = false;
    ...
    public void setAPerformanceValue(AcyclicCoordinates parent, AcyclicCoordinates offspring, int gapSinceLastUpgrade)
    {
        this.MutationFitnessValue.setAMutationFitnessValue(parent, offspring, gapSinceLastUpgrade);
    }
    
    private void findMutationFitnessValue(short type, OnlineAcyclicCoordinates CGPGraph, int gapSinceLastUpgrade) throws CloneNotSupportedException, IOException
    {
        OnlineAcyclicCoordinates GraphToMutate = (OnlineAcyclicCoordinates) CGPGraph.clone();
        OnlineAcyclicCoordinates Copy;
        MutationPerformanceAcyclic simulationFitness;  
        ...
        if (type == MutationPopulationAcyclic.SMALL_SCALE & this.SmallScaleVariation.getOffsprings().size() == 0)
        {
            noOfOffspring = 0;
        } 
        else if (type == MutationPopulationAcyclic.LARGE_SCALE & this.LargeScaleVariation.getOffsprings().size() == 0)
        {
            noOfOffspring = 0;
        }
        else if (type == MutationPopulationAcyclic.RUIN_RECREATE & this.RuinRecreateVariation.getOffsprings().size() == 0)
        {
            noOfOffspring = 0;
        }
        ...
        { 
           simulationFitness = new MutationPerformanceAcyclic(4);
           try 
           {
                switch (type)
                {   
                    case MutationPopulationAcyclic.SMALL_SCALE:    GraphToMutate.setVariarionOperator(this.SmallScaleVariation.getOffsprings().get(i));
                                                            break;
                    case MutationPopulationAcyclic.LARGE_SCALE:    GraphToMutate.setVariarionOperator(this.LargeScaleVariation.getOffsprings().get(i));
                                                            break;
                    case MutationPopulationAcyclic.RUIN_RECREATE:  GraphToMutate.setVariarionOperator(this.RuinRecreateVariation.getOffsprings().get(i));
                                                            break;                                    
                }
            }
          ...
          //  System.out.println("***Find" + GraphToMutate.toString());
            ...
            {
                Copy = (OnlineAcyclicCoordinates) GraphToMutate.clone();
    ...
    {
        if (type == MutationPopulationAcyclic.SMALL_SCALE)
        {
            ...
            if (this.bestFitnessValue < this.SmallScaleVariation.getParents().get(0).getAFitnessValue(0))
            {
               ...
               this.typeOfMutation =  MutationPopulationAcyclic.SMALL_SCALE;
            }
                
        }
        else if (type == MutationPopulationAcyclic.LARGE_SCALE)
        {
        ...
        else if (type == MutationPopulationAcyclic.RUIN_RECREATE)
        {
           this.RuinRecreateVariation.hyperPromotePositive();
            if (this.bestFitnessValue < this.RuinRecreateVariation.getParents().get(0).getAFitnessValue(0))
            {
               ...
               this.typeOfMutation =  MutationPopulationAcyclic.RUIN_RECREATE;
            }
    ...
    //    System.out.println("type " + type + "fitness value " + fitnessValue);
        switch (type)
        {
            case MutationPopulationAcyclic.SMALL_SCALE:   
                                                   ...
                                                   break;
            case MutationPopulationAcyclic.LARGE_SCALE:   this.LargeScaleVariation.getOffsprings().get(i).setAFitnessValue(0,fitnessValue);
            break;
            case MutationPopulationAcyclic.RUIN_RECREATE: this.RuinRecreateVariation.getOffsprings().get(i).setAFitnessValue(0,fitnessValue);
            break;
        }
    }
    ...
    private void setFitnessToAMutationOperatorParent(int type, int i, double fitnessValue)
    {
        switch (type)
        {
            case MutationPopulationAcyclic.SMALL_SCALE:   this.SmallScaleVariation.getParents().get(i).setAFitnessValue(0,fitnessValue);
            break;
            case MutationPopulationAcyclic.LARGE_SCALE:   this.LargeScaleVariation.getParents().get(i).setAFitnessValue(0,fitnessValue);
            break;
            case MutationPopulationAcyclic.RUIN_RECREATE: this.RuinRecreateVariation.getParents().get(i).setAFitnessValue(0,fitnessValue);
            break;
        }
    }
    ...
    public double getFitnessMutation()
    {
       switch (this.typeOfMutation)
        {
            case MutationPopulationAcyclic.SMALL_SCALE:   return this.SmallScaleVariation.getParents().get(0).getAFitnessValue(0);
                                                  
            case MutationPopulationAcyclic.LARGE_SCALE:   return  this.LargeScaleVariation.getParents().get(0).getAFitnessValue(0);
            
            case MutationPopulationAcyclic.RUIN_RECREATE: return this.RuinRecreateVariation.getParents().get(0).getAFitnessValue(0);
    ...
    public IterativeCoordinates getVariationOperator() throws CloneNotSupportedException
    {
        ...
        switch (this.typeOfMutation)
        {
            case MutationPopulationAcyclic.SMALL_SCALE:   return (IterativeCoordinates) this.SmallScaleVariation.getBestCGPgraph().clone();
                                                  
            case MutationPopulationAcyclic.LARGE_SCALE:   return (IterativeCoordinates) this.LargeScaleVariation.getBestCGPgraph().clone(); 
            
            case MutationPopulationAcyclic.RUIN_RECREATE: return (IterativeCoordinates)this.RuinRecreateVariation.getBestCGPgraph().clone();
  ...
  /*     if (this.SmallScaleVariation.getBestCGPgraphPositive().getAFitnessValue(0)>= this.bestFitnessValue)
    ...
    private int getSizeOfCurrentMutationOffsprings()
    {
     switch (this.typeOfMutation)
        {   
            case MutationPopulationAcyclic.SMALL_SCALE:    return this.SmallScaleVariation.getOffsprings().size();
            case MutationPopulationAcyclic.LARGE_SCALE:    return this.LargeScaleVariation.getOffsprings().size();
            case MutationPopulationAcyclic.RUIN_RECREATE:  return this.RuinRecreateVariation.getOffsprings().size();
            default: return 0;
        }
    }
    ...
    public String getCurrentMutation()
    {
     switch (this.typeOfMutation)
        {   
            case MutationPopulationAcyclic.SMALL_SCALE:    return "SMALL_SCALE " + this.SmallScaleVariation.getBestCGPgraph().getAlgorithm(0);
            case MutationPopulationAcyclic.LARGE_SCALE:    return "LARGE_SCALE " + this.LargeScaleVariation.getBestCGPgraph().getAlgorithm(0);
            case MutationPopulationAcyclic.RUIN_RECREATE:  return "RUIN_RECREATE " + this.RuinRecreateVariation.getBestCGPgraph().getAlgorithm(0);
